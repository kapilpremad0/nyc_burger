...
exports.login = async (req, res) => {
  ...
  try {
    ...
    if (!user || !["admin", "staff"].includes(user.user_type)) {
      return res.status(400).json({ error: "Invalid email or not allowed" });
    }
    ...
    req.session.admin = { id: user.id, email: user.email, role: user.user_type ,permissions: user.permissions };
